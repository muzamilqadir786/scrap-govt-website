...
#===========================#
# Instructions: 
# INSTALL MODULES (requests,bs4,lxml)
...
def secondary_point_of_contact(html):
    ...
    if html.xpath('//div[@id="so_formfield_dnf_class_values_procurement_notice__secondary_poc_"]'):
        ...
        if html.xpath('//div[@id="dnf_class_values_procurement_notice__primary_poc__widget"]/div[4]/text()')[0]:
...
def extract(url):
    ...
    data= {
    	...
    	# 'dnf_opt_template':'d7ZhS+mcdt/Gajtw+JsBazXxVYcDLoQW1MDkvvEnorEEQQXqMlNO+qihNxtVFxhn',
    	# 'dnf_opt_template_dir':'C4gsU7FM6NLSpB/FamlLE7G6WrxuiBuGRpBBjyvqt1KAkN/anUTlMWIUZ8ga9kY+',
    	# 'dnf_opt_subform_template':'PcsQSVZOI3aHWRpI4HomdmF719zd85B9',
    	'dnf_opt_finalize':'1',
    	'dnf_opt_mode':'update',
    	'dnf_opt_target':'',
    	'dnf_opt_validate':'1',
    	...
    	'dnf_class_values[procurement_notice][keywords]': 'Solar',
    	'dnf_class_values[procurement_notice][naics_code][]': '0025021',   
    }
        ...
        for rowdata in table_data.find_all('tr'): 
            ...
            if page_url:
                ...
                if(response.code == 200):
                    ...
                    # s = general_information(html)
                    # b = b = [re.sub('[\s]',' ',sa).encode('utf-8').strip() for sa in s]                   
                    # print b
                    # print ' '.join(''.join([re.sub('[\s]',' ',info).strip() for info in general_information(html)]).split())
                    # print general_information(html)
                    # break
                    # Field 37 to 40
